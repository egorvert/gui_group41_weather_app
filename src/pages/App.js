...
import SearchIcon from '../assets/icons/ui/searchIcon.svg';

// temp api key for previous days FLTVUZY62E39QKWE5SY7YW7GS
// second api key HJ662UQ43MYUPLGLYSMPMU9KP
...
function App() {
  ...
  // Format dates for the api request to get the previous days information
  ...
  // Assign use states for ease of setting for previous days sections
  ...
  // Setup use states for sunrise and sunset widgets
  ...
  let previousurl  = `https://weather.visualcrossing.com/VisualCrossingWebServices/rest/services/timeline/${citylat},${citylon}/${threedaysago}/${yesterday}?unitGroup=metric&include=days&key=PQ6XCBAJCZXKCCZDQVWWVLERZ&contentType=json`
  
  // Function to set longitude and latitude of the searched city as it is needed for the hourly updates  
        ...
        if ((result.status !== 404) || (result.status !== 400)) {
          // If api call is bad due to no city existing does not call the api
            result.json().then(json => {
              // Sets the city longitude and latitude with extra error handling
              try {
  ...
  // Function to set the bottom widget data
  ...
  useEffect(() => {
    // Async function to make sure app waits for data to be fetched before rendering it
    const fetchData = async () => {
      await setcoords() // Sets coords and calls other functions
      let badstatus = false
      let result = await fetch(url)
      if ((result.status === 404) || (result.status === 400) ){
        result = await fetch(url_backup) // If city is invalid resets to default backup option
        badstatus = true
    }     
    ...
    // If there has been an error or invalid input, set the city back to default
    if(badstatus === true){
      setCity('London')
    }  
    }
  ...
  return (
    <div className="App">
      <div className='search-wrapper'>
        ...
        <input type="text" placeholder="Search for a new location" onKeyDown={onChangeHandler} />
      </div>
        <div className='Sun'>
          <Sun suninfo={suninfo} />
        </div>
